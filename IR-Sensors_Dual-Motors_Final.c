...
// ** CALIBRATION BUTTON ** //
		...
		if (last_button && (b==0)){ // has the button bit gone from high to low
			...
			// Record average sensor value
			averageSensorValue = (left + right)/2;
...
// ** END CALIBRATION BUTTON ** //
		...
		/** follows light **/
		if (left > right && right < 350 && motorOn == 1) {  // if right sensor is bright, left dark
			TA1CCR1 = 8888; // right wheel
			TA0CCR1 = 0; // left wheel
		}
		else if (right > left && left < 350 && motorOn == 1) { // if left sensor is bright, right dark
			TA0CCR1 = 7777; // left wheel
			TA1CCR1 = 0; // right wheel
		}
		else if ((right > (averageSensorValue+100) || left > (averageSensorValue+100)) && motorOn == 1) { 
		// if sensors touch over black tape, stop car
			motorOn = 0;
			TA0CCR1 = 0;
			TA1CCR1 = 0;
		}
		else if ( motorOn == 1){
			TA0CCR1 = 5555; // left wheel
			TA1CCR1 = 6666; // right wheel
		}
		/** END LOGIC ABOVE **/
}
